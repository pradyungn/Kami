...
/// Fake implementation of [ProviderAPI].
class FakeProvider extends ChangeNotifier implements ProviderAPI<FakeItem> {
  bool _isLoggedIn = false;
  List<FakeItem> _storedTexts = [];
  List<FakeItem> _storedTextsRemote = [
    FakeItem(text: "Lorem ipsum dolor sit amet", title: "Very long book"),
    FakeItem(
      text: "sldk sdlfh sldkf smkdf sbdln sndfklnds kd fdkf ",
      title: "Short article",
    ),
    FakeItem(text: "item", title: "idk", summary: "This has a summary!"),
  ];
...
Have you experienced this before? To be honest, I don’t have the answers. Same failure, yet different responses. Who is right and who is wrong? Neither. Each person has a different mindset that decides their outcome. It was at that point their biggest breakthrough came. Perhaps all those years of perseverance finally paid off. If you settle for less, you will receive less than you deserve and convince yourself you are justified to receive it. It must come from within you. Gnaw away at your problems until you solve them or find a solution. Where are you settling in your life right now? Could you be you playing for bigger stakes than you are? Vision + desire + dedication + patience + daily action leads to astonishing success. Success is a fickle and long game with highs and lows. So become intentional on what you want out of life. Commit to it. Nurture your dreams. Don’t leave your dreams to chance.
  ...
  @override
  Future<FakeItem> storeText(FakeItem text) async {
    // Generate random ID
    String id = Uuid().v4();
    FakeItem newItem = text._hydrate(id, this);
    _updateItem(newItem);
    return newItem;
  }

  @override
  List<FakeItem> get storedTexts {
    return _storedTexts;
  }
  ...
  Future<void> fetchStoredTexts() async {
    ...
    _storedTexts =
        _storedTextsRemote.map((e) => e._hydrate(Uuid().v4(), this)).toList();
    notifyListeners();
    print('Fetched stored texts');
  }

  Future<void> _deleteText(FakeItem text) async {
    if (!_isLoggedIn) return;
...
class FakeItem extends TextItem {
  final FakeProvider _provider;
  ...
  Future<void> update() async {
    _provider._updateItem(this);
  }

  @override
  Future<void> delete() async {
    _provider._deleteText(this);
  }

  FakeItem _hydrate(String id, FakeProvider provider) {
    return FakeItem._(
      provider: provider,
      id: id,
      text: text,
